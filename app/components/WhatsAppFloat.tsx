'use client';

import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';

const WHATSAPP_URL = 'https://whatsapp.com/channel/0029VbCyw0P9mrGciiEpD71G';

const WhatsAppIcon = ({ className }: { className?: string }) => (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
);

export default function WhatsAppFloat() {
    const [visible, setVisible] = useState(false);
    const [dismissed, setDismissed] = useState(false);

    useEffect(() => {
        const timer = setTimeout(() => setVisible(true), 2000);
        return () => clearTimeout(timer);
    }, []);

    if (dismissed) return null;

    return (
        <AnimatePresence>
            {visible && (
                <motion.div
                    className="fixed bottom-24 right-6 z-[9999] flex flex-col items-end gap-2"
                    initial={{ opacity: 0, x: 80, scale: 0.8 }}
                    animate={{ opacity: 1, x: 0, scale: 1 }}
                    exit={{ opacity: 0, x: 80, scale: 0.8 }}
                    transition={{ type: 'spring', stiffness: 260, damping: 20 }}
                >
                    {/* Dismiss button */}
                    <button
                        onClick={() => setDismissed(true)}
                        aria-label="Dismiss WhatsApp prompt"
                        className="w-5 h-5 rounded-full bg-slate-700/80 text-white/70 hover:bg-slate-600 hover:text-white transition-colors flex items-center justify-center text-xs leading-none mr-0.5"
                    >
                        ✕
                    </button>

                    {/* Main floating CTA */}
                    <a
                        href={WHATSAPP_URL}
                        target="_blank"
                        rel="noopener noreferrer"
                        id="whatsapp-float-cta"
                        aria-label="Join our WhatsApp channel"
                        className="group relative flex items-center gap-3 bg-[#25D366] text-white rounded-full shadow-2xl cursor-pointer overflow-hidden
                                   pl-4 pr-5 py-3
                                   hover:shadow-[0_0_32px_8px_rgba(37,211,102,0.45)]
                                   transition-all duration-300"
                    >
                        {/* Pulsing ring */}
                        <span className="absolute inset-0 rounded-full animate-ping bg-[#25D366]/40 pointer-events-none" />
                        <span className="absolute inset-0 rounded-full animate-ping bg-[#25D366]/20 pointer-events-none [animation-delay:0.4s]" />

                        <WhatsAppIcon className="w-7 h-7 flex-shrink-0 relative z-10 drop-shadow" />

                        <span className="relative z-10 flex flex-col leading-tight">
                            <span className="text-[10px] font-bold uppercase tracking-widest opacity-80">Stay in the loop</span>
                            <span className="text-sm font-black uppercase tracking-wide font-oswald whitespace-nowrap">
                                Join our Channel
                            </span>
                        </span>

                        {/* Arrow */}
                        <svg className="w-4 h-4 relative z-10 opacity-70 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 8l4 4m0 0l-4 4m4-4H3" />
                        </svg>
                    </a>
                </motion.div>
            )}
        </AnimatePresence>
    );
}
