...
export default function Navbar({ onRegisterClick }: { onRegisterClick?: () => void }) {
    ...
    // Close desktop register menu when clicking outside
    useEffect(() => {
    ...
    return (
        ...
        >
            <div className="container mx-auto px-4 md:px-8 h-12 md:h-14 flex items-center justify-between relative z-[101]">
                ...
                >
                    <Image
                        ...
                        width={60}
                        height={60}
                        className={`w-auto h-10 md:h-12 object-contain transition-transform duration-300 group-hover:scale-105 ${!scrolled && !isMenuOpen ? 'brightness-100' : ''
                            }`}
                        priority
                    />
                </Link>
                ...
                {/* Desktop Navigation */}
                <nav className={`hidden lg:flex items-center gap-8 text-[13px] font-black uppercase tracking-widest transition-colors duration-300 ${scrolled ? 'text-slate-800' : 'text-white'
                    }`}>
                    ...
                    <Link href="#highlights" className="hover:text-biro-blue-dark transition-colors relative after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-biro-blue after:transition-all hover:after:w-full">Highlights</Link>
                    <Link href="#tickets" className="hover:text-biro-blue-dark transition-colors relative after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-biro-blue after:transition-all hover:after:w-full">Tickets</Link>
                    <div className="relative">
                        ...
                        <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-4 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden transition-all duration-300 transform z-[110] ${isRegisterOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-2'
                            }`}>
                            <div className="p-2">
                                <button
                                    onClick={() => {
                                    ...
                                    className="w-full text-left block px-4 py-3 text-slate-700 hover:bg-slate-50 hover:text-biro-blue rounded-xl font-black uppercase tracking-widest text-[11px] transition-colors"
                                >
                                    Attend Event
                                </button>
                    ...
                    <Link href="#faq" className="hover:text-biro-blue-dark transition-colors relative after:content-[''] after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-biro-blue after:transition-all hover:after:w-full">FAQ</Link>
                ...
                <div className="flex items-center gap-4">
                    <button
                        ...
                        className={`hidden sm:flex px-6 py-2.5 rounded-full font-black uppercase tracking-widest text-[11px] transition-all duration-300 active:scale-95 shadow-md hover:shadow-lg ${scrolled || isMenuOpen
                    ...
                    <button
                        ...
                        className={`lg:hidden p-2 cursor-pointer transition-colors duration-300 ${isMenuOpen || scrolled ? 'text-biro-blue' : 'text-white'
                            }`}
            ...
            {/* Mobile Menu Overlay */}
            <div
                className={`fixed inset-0 bg-slate-900/60 backdrop-blur-sm lg:hidden transition-all duration-500 ease-in-out z-[200] ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
                    }`}
                onClick={() => setIsMenuOpen(false)}
            >
                <div
                    className={`absolute top-0 right-0 h-screen w-full max-w-sm bg-white shadow-2xl transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) flex flex-col ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
                        }`}
                ...
                >
                    ...
                    <div className="flex flex-col p-8 gap-5 overflow-y-auto">
                        <Link
                            href="#about"
                            className={`text-2xl font-black font-righteous uppercase tracking-tight text-slate-800 hover:text-biro-blue transition-all duration-300 transform ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}
                        ...
                        <div
                            className={`flex flex-col gap-3 transition-all duration-300 transform ${isMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}
                            style={{ transitionDelay: '250ms' }}
                        >
                            <button
                                onClick={() => setIsMobileRegisterOpen(!isMobileRegisterOpen)}
                                className="flex items-center justify-between text-2xl font-black font-righteous uppercase tracking-tight text-slate-800 hover:text-biro-blue text-left w-full"
                            >
                                Register
                            ...
                            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isMobileRegisterOpen ? 'max-h-48' : 'max-h-0'}`}>
                                <div className="flex flex-col gap-3 pl-4 pt-3 border-l-2 border-slate-100">
                                    <button
                                        ...
                                        className="text-left text-xl font-bold text-slate-800 hover:text-biro-blue"
                                    >
                                        Attend Event
                                    </button>
                        ...
                        <div className={`mt-4 pt-8 border-t border-slate-100 transition-all duration-500 transform ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`} style={{ transitionDelay: '450ms' }}>
                            <button
                                ...
                                className="w-full flex items-center justify-center py-4 bg-biro-blue text-white text-center font-black font-righteous uppercase tracking-widest text-lg rounded-xl shadow-lg shadow-biro-blue/20 hover:bg-biro-blue-dark transition-all active:scale-95"
                            >
                                Register Now
                            </button>
                        </div>
