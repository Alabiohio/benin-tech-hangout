    ...
    return (
        <footer id="contact" className="relative z-10 bg-white text-white py-4 overflow-hidden">
            ...
            <div className="container mx-auto relative z-10 px-4 md:px-8 pt-8">
                ...
                <div className="relative my-6 mb-12 rounded-2xl overflow-hidden border shadow md:-translate-x-26 md:pl-21 shadow-lg bg-[#ffffff]">
                    <div className="relative flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-4">
                        <div className="flex items-center gap-4">
                            <div className="relative flex-shrink-0">
                                <div className="w-10 h-10 rounded-full bg-biro-blue border border-biro-blue flex items-center justify-center text-white transition-all shadow-md">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-black uppercase font-oswald text-biro-blue">Join our WhatsApp channel for live updates &amp; announcements</p>
                            </div>
                        </div>
                        ...
                        <a
                            ...
                            className="flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-full bg-biro-blue text-white font-black font-oswald uppercase tracking-widest text-sm hover:bg-biro-blue hover:border hover:border-3 hover:border-bg-biro-blue hover:bg-transparent hover:text-biro-blue-dark transition-all duration-300 group"
                        >
                            Join Channel
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mb-12 text-left">
                    {/* Brand & Socials */}
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex gap-2">
                                {[
                                    ...
                                    <Link key={social.id} href={social.url} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-biro-blue border border-biro-blue flex items-center justify-center text-white hover:bg-biro-blue-dark hover:border-biro-blue-dark transition-all shadow-md">
                                        <span className="sr-only">{social.id}</span>
                                        {social.icon}
                                    </Link>
                                ))}
                            </div>
                            <span className="text-[15px] font-black uppercase tracking-[0.3em] text-slate-500">@benintechfest</span>
                        </div>
                    </div>

                    {/* Quick Links */}
                    <div className="flex flex-col items-start gap-4">
                        <span className="text-[16px] font-bold uppercase tracking-[0.2em] text-slate-500">Quick Links</span>
                        <nav className="flex flex-col sm:grid sm:grid-cols-2 gap-x-8 gap-y-3">
                            {['Volunteer', 'Exhibitor', 'Tickets', 'Sponsor', 'Privacy Policy'].map((item) => (
                                <Link key={item} href={item === 'Privacy Policy' ? '/privacy-policy' : `/${item.toLowerCase()}`} className="text-[15px] font-black uppercase tracking-widest text-slate-400 hover:text-highlight-yellow font-cabinet-grotesk transition-colors text-left">
                                    {item}
                                </Link>
                            ))}
                        </nav>
                    ...
                    <div className="flex flex-col items-start lg:items-end gap-4 text-left lg:text-right">
                        <span className="text-[16px] font-bold uppercase tracking-[0.2em] text-slate-500">Contact Us</span>
                        <div className="flex flex-col gap-2">
                            <a href="mailto:benintechfest@gmail.com" className="text-[15px] font-black uppercase tracking-widest text-slate-400 hover:text-highlight-yellow font-cabinet-grotesk transition-colors">benintechfest@gmail.com</a>
                            <a href="tel:+2347012341561" className="text-[15px] font-black uppercase tracking-widest text-slate-400 hover:text-highlight-yellow font-cabinet-grotesk transition-colors">+234 701 234 1561</a>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center py-6 md:py-4 mt-4 md:mt-0">
                    <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-6 text-center md:text-left">
                        <p className="text-slate-500 text-[15px] md:text-[15px] font-black uppercase tracking-widest">&copy; {new Date().getFullYear()} Benin Tech Fest</p>
                    </div>
                </div>
            </div>
        </footer>
