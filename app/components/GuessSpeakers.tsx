...
const GuessSpeakers = () => {
    ...
    return (
        <section className="py-24 bg-gradient-to-br from-[#0B132B] to-[#020617] relative overflow-hidden border-t border-white/[0.02]">
            ...
            <div className="container mx-auto px-6 relative z-10">
                <div className="text-center mb-16" data-aos="fade-up">
                    ...
                    <h2 className="text-4xl md:text-7xl font-black font-righteous text-white mb-6 italic">
                        The <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">Speakers</span>
                    </h2>
                    <p className="text-white/60 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
                        Meet the brightest minds shaping the tech landscape. More icons to be revealed soon.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
                    {speakersList.map((speaker, idx) => (
                        <div
                            ...
                            className="group relative h-[450px] rounded-[2.5rem] overflow-hidden border border-white/10 bg-white/5 backdrop-blur-sm transition-all duration-500 hover:border-blue-500/50 hover:shadow-[0_0_30px_rgba(59,130,246,0.2)]"
                        >
                            {speaker.revealed && speaker.image ? (
                                <>
                                    <Image
                                        src={speaker.image}
                                        alt={speaker.name}
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-8 bg-gradient-to-t from-[#020617] via-[#020617]/40 to-transparent">
                                <div className={`h-2 rounded-full mb-4 transition-all duration-500 ${speaker.revealed ? 'w-16 bg-highlight-yellow' : 'w-12 bg-blue-500/30 group-hover:w-20 group-hover:bg-blue-500'}`}></div>
                                <h3 className={`text-2xl font-black font-righteous uppercase tracking-tighter transition-all duration-500 ${speaker.revealed ? 'text-white' : 'text-white/20 group-hover:text-white/40'}`}>
                                    {speaker.name}
                                </h3>
                                <p className={`text-xs font-bold uppercase tracking-widest transition-all duration-500 ${speaker.revealed ? 'text-highlight-yellow' : 'text-white/10 group-hover:text-blue-400/40'}`}>
                                    {speaker.role}
                                </p>
                            </div>
